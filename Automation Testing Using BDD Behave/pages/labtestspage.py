...
class LabTestsPage(BasePage):
    ...
    def proceed_to_pay(self):
        ...
        time.sleep(3)
        try:
            # Find the modal explicitly
            xpath_modal = "//div[@role='dialog' or contains(@class, 'modal') or contains(@class, 'MuiDrawer-paper')]"
            modals = self.driver.find_elements(By.XPATH, xpath_modal)
            
            target_btn = None
            if modals:
                # Target the button inside the modal
                modal = modals[-1]
                btns = modal.find_elements(By.XPATH, ".//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'review cart') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'proceed') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'continue')]")
                valid_btns = [b for b in btns if b.is_displayed()]
                if valid_btns:
                    # Prioritize the button that explicitly says "Review Cart"
                    review_btns = [b for b in valid_btns if 'review cart' in (b.text or '').lower()]
                    target_btn = review_btns[-1] if review_btns else valid_btns[-1]
            
            if not target_btn:
                # Fallback to general page button
                xpath_all = "//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'review cart') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'proceed') or contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'continue')]"
                btns = self.driver.find_elements(By.XPATH, xpath_all)
                valid_btns = [b for b in btns if b.is_displayed()]
                if valid_btns:
                    review_btns = [b for b in valid_btns if 'review cart' in (b.text or '').lower()]
                    target_btn = review_btns[-1] if review_btns else valid_btns[-1]
            ...
            if target_btn:
                self.logger.info(f"Found Review Cart button: {target_btn.text}")
                ...
                try:
                    ...
                    self.logger.warning("ActionChains click failed, trying JS click")
                    self.driver.execute_script("arguments[0].click();", target_btn)
            else:
                self.logger.warning("Could not find Review Cart button.")
        ...
        try:
            xpath_ptp = "//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'proceed to pay')]"
            btns_ptp = self.driver.find_elements(By.XPATH, xpath_ptp)
            valid_btns_ptp = [b for b in btns_ptp if b.is_displayed()]
            if valid_btns_ptp:
                target_ptp = valid_btns_ptp[0]
                self.logger.info(f"Found Proceed to Pay button: {target_ptp.text}")
                self.driver.execute_script("arguments[0].scrollIntoView({block: 'center'});", target_ptp)
                time.sleep(1)
                try:
                    ActionChains(self.driver).move_to_element(target_ptp).click().perform()
                except Exception:
                    self.driver.execute_script("arguments[0].click();", target_ptp)
            else:
                self.logger.warning("Proceed to Pay button not found.")
        except Exception as e:
